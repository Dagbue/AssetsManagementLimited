...
export default {
  name: "DashBoardListOfUsersViewDemo",
  data()  {
    return {
      contacts: [],
      SelectEmail: "",
  ...
  async created() {
    const querySnapshot = await getDocs(collection(db, "listOfUsersDemo"));
  ...
  methods: {
    ...
    async update() {
      ...
      const docRef = doc(db, "listOfUsersDemo", this.SelectEmail);
      ...
      const docRef2 = doc(db, this.SelectEmail, "UserDemo");
