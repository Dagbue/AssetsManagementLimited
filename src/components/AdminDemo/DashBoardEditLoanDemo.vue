      ...
      <div class="table" v-if="this.contacts.length >0">
        <table>
          ...
          <tr>
            ...
            <td>$ {{child.prepaymentFee}}</td>
            <td>{{ child.statusLoan }}</td>
            <td>{{child.loanPlan}}</td>
          </tr>
          </tbody>
...
export default {
  ...
  async created() {
    const querySnapshot2 = await getDocs(collection(db, "loansDemo"));
  ...
  methods: {
    async update1() {
      await setDoc(doc(db, "loansDemo" ,this.SelectEmail ), {
