...
export default {
  name: "DashBoardDepositRequestsViewDemo",
  data () {
    return {
      SelectEmail: "",
      history: [],
  ...
  async created() {
    const HistoryRef = ref(database, "/DepositRequestsDemo");
  ...
  methods: {
    async update() {
      ...
      await remove(ref(database, "DepositRequestsDemo/"))
          .then(async () => {
            await Swal.fire({
              icon: 'success',
              title: 'Success',
